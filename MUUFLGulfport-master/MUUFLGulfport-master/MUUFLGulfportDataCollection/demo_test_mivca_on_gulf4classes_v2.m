% =========================================================================
% demo_test_mivca_on_gulf4classes.m 的修改版
% 改动：
%   1. Figure 103 xlim 改为 [0, 1e-3]（原先是 [1e-5, 1e-3]）
%   2. 将 Figure 103 的 ROC 数据导出为 mat 文件
%   3. 其他所有逻辑与原脚本保持完全一致
% =========================================================================
...
ace_out_pg = ace_detector(hsi_img,target_sigs(:,1),hsi.valid_mask);
smf_out_pg = smf_detector(hsi_img,target_sigs(:,1),hsi.valid_mask);
...
% ===== 修改点 1：Figure 103 xlim 改为 [0, 1e-3] =====
figure(103);
...
max_far = 1e-3;
auc = auc_upto_far(max_far, ace_score);
...
save('Figure103_ROC_data.mat', '-struct', 'roc_data');
disp('已保存: Figure103_ROC_data.mat');

% 同时保存原始 score cell，方便后续直接用 PlotBullwinkleRoc 重画
save('Figure103_score_backup.mat', 'score');
disp('已备份 score 结构: Figure103_score_backup.mat');
