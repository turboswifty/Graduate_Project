% =========================================================================
% eFUMI 端元 ACE 检测 ROC 计算与导出脚本
% 只计算并保存 eFUMI 预测端元的 ROC 数据，方便他人复用绘图
% =========================================================================
...
fprintf('eFUMI 端元 ACE 检测 AUC = %.4f\n', AUC);

%% 5. 保存为 mat 文件，方便他人复用绘图
% 保存所有关键变量，别人加载后可以直接 plot(FPR, TPR)
save('ROC_eFUMI_ACE.mat', 'FPR', 'TPR', 'AUC', 'ace_scores', 'labels_point', 'E_efumi', 'mu_bg');
disp('已保存 ROC 数据到 ROC_eFUMI_ACE.mat');
disp('  包含变量: FPR, TPR, AUC, ace_scores, labels_point, E_efumi, mu_bg');

%% 6. 画图（只画 eFUMI）
figure('Name', 'eFUMI ACE ROC', 'Position', [150, 150, 600, 500]);
...
title(sprintf('eFUMI ACE Detection ROC (AUC = %.4f)', AUC), 'FontSize', 13, 'FontWeight', 'bold');
legend({'eFUMI ACE', 'Random Guess'}, 'Location', 'SouthEast', 'FontSize', 11);
grid on;
set(gca, 'FontSize', 11);
