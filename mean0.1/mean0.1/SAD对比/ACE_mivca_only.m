% =========================================================================
% MI-VCA 端元 ACE 检测 ROC 计算与导出脚本
% 只计算并保存 MI-VCA 预测端元的 ROC 数据，方便他人复用绘图
% =========================================================================
...
fprintf('MI-VCA 端元 ACE 检测 AUC = %.4f\n', AUC);

%% 5. 保存为 mat 文件，方便他人复用绘图
% 保存所有关键变量，别人加载后可以直接 plot(FPR, TPR)
save('ROC_MIVCA_ACE.mat', 'FPR', 'TPR', 'AUC', 'ace_scores', 'labels_point', 'E_mivca', 'mu_bg');
disp('已保存 ROC 数据到 ROC_MIVCA_ACE.mat');
disp('  包含变量: FPR, TPR, AUC, ace_scores, labels_point, E_mivca, mu_bg');

%% 6. 画图（只画 MI-VCA）
figure('Name', 'MI-VCA ACE ROC', 'Position', [150, 150, 600, 500]);
...
title(sprintf('MI-VCA ACE Detection ROC (AUC = %.4f)', AUC), 'FontSize', 13, 'FontWeight', 'bold');
legend({'MI-VCA ACE', 'Random Guess'}, 'Location', 'SouthEast', 'FontSize', 11);
grid on;
set(gca, 'FontSize', 11);
